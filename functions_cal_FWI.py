...
def DryingFactor(Latitude, Month, cfg):
    ...
    if cfg.adjust_DryingFactor == "original":
    	...
    	retVal = np.where(Latitude > 0, LfN[Month - 1], LfS[Month - 1])
    
    elif cfg.adjust_DryingFactor == "NSHeq":
    	# setting drying factor at different latitudes
    	LfN_array = np.array(LfN)
    	LfS_array = np.array(LfS)
    	Lfeq_array = np.full_like(Latitude, Lfeq)
    	ind_LatG20 = Latitude > 20
    	ind_LatL20 = (Latitude > -20) & (Latitude <= 20)
    	retVal = np.where(ind_LatG20, LfN_array[Month - 1], np.where(ind_LatL20, Lfeq_array, LfS_array[Month - 1]))
    else:
